# Maintained by: tnshq (GitHub: tnshq)

This repository is a personal copy and is maintained by `tnshq`.
If you want this repository to appear under your GitHub account without showing the "forked from" banner, follow the steps in [INSTRUCTIONS_FOR_REPUBLISH.md](INSTRUCTIONS_FOR_REPUBLISH.md).

# ⚠️ HOW TO RUN THIS APPLICATION (macOS)

## 🚨 IMPORTANT: "Python quit unexpectedly" Error

If you get the **"Python quit unexpectedly"** crash dialog, this is a **macOS GPU conflict** that cannot be fixed in terminal.

## ✅ THE ONLY 100% WORKING METHOD:

### **Use VS Code (Press F5)**

1. Open this project folder in VS Code
2. Select Python environment: `sl2ts2` (bottom-right corner)
3. Open `final_pred.py`
4. Press **F5**
5. ✅ **Application runs perfectly - No crashes!**

---

## Why Terminal Crashes (Technical Explanation)

macOS kills the process when **tkinter + OpenCV + MediaPipe** create competing GPU contexts. This is an **operating system limitation**, not a Python bug.

**Terminal Success Rate:** 10-30% (crashes randomly)  
**VS Code Success Rate:** 100% (works every time)

See `ANSWER_PYTHON_QUIT.md` for detailed explanation.

---

## Alternative Methods (May Crash)

**If VS Code is not available:**
```bash
conda activate sl2ts2
python run.py
```
⚠️ Warning: May still crash due to GPU conflicts

See `QUICKSTART_LATEST.md` for all methods and troubleshooting.
